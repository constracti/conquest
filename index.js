	...
	function row(station) {
		...
		if (station !== null) {
			element_list.push(n({
				...
				click: async () => {
					 ...
					 * @type {Station[]}
					 */
					const result = await api.post('station_delete', form_data);
		...
		element_list.push(n({
			...
			submit: async event => {
				 ...
				 * @type {Station[]}
				 */
				const result = await api.post(station !== null ? 'station_update' : 'station_insert', form_data);
				state.station_list = result;
				spinner_div.classList.add('d-none');
				render();
			},
	...
	function row(team) {
		...
		if (team !== null) {
			element_list.push(n({
				...
				click: async () => {
					 ...
					 * @type {Team[]}
					 */
					const result = await api.post('team_delete', form_data);
		...
		element_list.push(n({
			...
			submit: async event => {
				 ...
				 * @type {Team[]|null}
				 */
				const result = await api.post(team !== null ? 'team_update' : 'team_insert', form_data);
				if (result === null) {
...
function render_player() {
	...
	function row(player) {
		...
		if (player !== null) {
			...
			element_list.push(team_badge(team));
		} else {
			element_list.push(n({
				class: 'm-1 flex-grow-1',
			}));
		...
		if (player !== null) {
			element_list.push(n({
				...
				click: async () => {
					 ...
					 * @type {Player[]}
					 */
					const result = await api.post('player_delete', form_data);
		...
		element_list.push(n({
			...
			submit: async event => {
				 ...
				 * @type {Player[]|null}
				 */
				const result = await api.post(player !== null ? 'player_update' : 'player_insert', form_data);
				if (result === null) {
...
import_form.addEventListener('submit', async event => {
	 ...
	 */
	const result = await api.post('player_import', form_data);
	if ('error' in result) {
